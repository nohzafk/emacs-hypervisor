...
fn generated_init_elisp() -> String {
    ...
    output.push_str(&stable_content_hash(&body));
    output.push('\n');
    output.push_str(
        ";; This file was generated by `emacs-hypervisor init'. Edit config.org or config.el instead.\n\n",
    );
...
fn run_init(home: PathBuf) -> Result<(), String> {
    ...
    println!("Next steps:");
    println!("- create {} (recommended)", home.join("config.org").display());
    println!("- or create {}", home.join("config.el").display());
    println!(
        "- optionally create {}",
...
mod tests {
    ...
    #[test]
    fn init_writes_single_bundled_init_el() {
        let home = unique_test_home();
        let result = run_init(home.clone());
