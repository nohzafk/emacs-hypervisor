...
(def protocol
  {:await-response
   ...
   :send-event
   (fn [topic payload]
     (push stub-sent-events {:topic topic :payload (wire-protocol:to-wire payload)}))
   :send-report
   (fn [stage phase payload]
     (push stub-sent-events
           {:topic :report
            :payload {:stage stage
                      :phase phase
                      :payload (wire-protocol:to-wire payload)}}))
   :send-eval-form-string-request
   (fn [id form-string]
...
(def installed-packages
  (map
   (fn [{:name name :deps deps :repo repo :host host :branch branch :tag tag :ref ref :local local :lisp-dir lisp-dir}]
     {:name name
      :deps deps
      :repo repo
      :host host
...
(assert
 (= (wire-protocol:sexp-string (list :ok true :payload nil :duration-ms 1.5))
    "(:ok true :payload nil :duration-ms 1.5)")
 "protocol prints primitive scalars without unsafe conversion")
(assert
...
(let [probe-message
      (wire-protocol:make-request
       10
       :eval
       (list :form (list 'executable-find "git")))]
  (assert
   (= (wire-protocol:sexp-string probe-message)
...
(let* [probe-units
       (list
        {:name "has-sh" :executable (list "sh")}
        {:name "missing-bin" :executable (list "definitely-not-installed-command")})
       probe-env
       (list {:name "PATH" :value "/bin:/usr/bin"})
       {:next-id next-id :reports reports}
       (preflight:probe-executables probe-units 10 probe-env)
  ...
  (assert
   (= (get missing-bin :missing) (list "definitely-not-installed-command"))
   "path preflight reports missing executable"))
(println "  0b. executable PATH preflight: ok")

(let* [{:reports installed-package-reports}
       (policy:derive-package-reports installed-packages)
       installed-plan
       (planning:derive-package-plan installed-packages installed-package-reports)
  ...
  (assert
   (= (length (planning:plan-items installed-plan)) 0)
   "installed packages are omitted from install plan"))
...
(assert
 (string/contains?
  (wire-protocol:plist-get (get (get stub-sent-events 0) :payload) :message)
  "has no content hash")
...
(let* [{:reports planned-package-reports}
  ...
  (assert (= (graph:entry-field (graph:entry-field invalid-root :details) :missing) (list "ghost-pkg")) "boot-policy invalid package details")
  ...
  (assert (= (graph:entry-field (graph:entry-field blocked-unit :details) :blockers) (list "invalid-root")) "boot-policy blocked unit details")
  (assert (= (graph:entry-field invalid-after :reason) :missing-after-units) "boot-policy invalid after reason")
  (assert (= (graph:entry-field (graph:entry-field invalid-after :details) :missing) (list "ghost-unit")) "boot-policy invalid after details")
  ...
  (assert (= (graph:entry-field (graph:entry-field preflight-bad :details) :env) (list "HYPERVISOR_MISSING_ENV")) "boot-policy preflight env details")
  (assert (= (graph:entry-field (graph:entry-field preflight-bad :details) :executable) (list "definitely-not-installed-command")) "boot-policy preflight executable details"))
...
(let* [{:reports planned-package-reports}
  ...
  (let* [report-event
         (first
          (filter
           (fn [event] (= (get event :topic) :report))
           stub-sent-events))
    ...
    (assert
     (= (length report-items) (length planned-package-reports))
     "startup report payload preserves count")
    (assert
     (= (wire-protocol:plist-get first-report :name) "ui-pkg")
     "startup report payload preserves report names")
    (assert
     (= (wire-protocol:plist-get first-report :status) :ok)
     "startup report payload preserves report status")
    (assert
     (= (wire-protocol:plist-get first-report :reason) :ready)
     "startup report payload preserves report reason")
    (assert
     (not
      (nil?
       (wire-protocol:sexp-string
        (wire-protocol:make-report-message
         :planned
         :packages
         report-items))))
  ...
  (assert
   (= (planning:plan-names package-plan)
      (list "core-pkg" "ui-pkg" "runtime-fail-pkg" "runtime-fail-dependent"))
  ...
  (assert
   ...
   "planning unit order")
  (assert (= (length (filter (fn [event] (= (get event :topic) :plan)) stub-sent-events))
             2)
          "planning emits two plan messages"))
(println "  2. planning: ok")

(reset-stub-state)
(reporting:emit-report-logs
 "unit"
 (list
  (graph:make-report
   "ok-unit"
  ...
  (graph:make-report
   ...
   :skipped
   :blocked-by-package
   {:blockers (list "bad-package")
    :body '(progn (lambda () :opaque))})
  (graph:make-report
   "other-bad-detail-unit"
   :invalid
   :cycle
   {:members (list "other-bad-detail-unit")
    :body '(progn (lambda () :opaque))})))
(let* [event (get stub-sent-events 0)
  ...
  (assert
   (= (wire-protocol:plist-get payload :message)
      "unit reports not ok: 2 of 3")
...
(reporting:emit-report-message
 ...
 (list
  (graph:make-report
   ...
   :skipped
   :blocked-by-package
   (list :blockers (list "bad-package")))))
(let* [event
       (first
        (filter
         (fn [event] (= (get event :topic) :report))
  ...
  (assert
   (= (wire-protocol:plist-get report :name) "opaque-detail-unit")
   "report message preserves report name")
  (assert
   (= (wire-protocol:plist-get report :status) :skipped)
   "report message preserves report status")
  (assert
   (= (wire-protocol:plist-get report :reason) :blocked-by-package)
   "report message preserves report reason")
  (assert
   (= (wire-protocol:plist-get details :blockers) (list "bad-package"))
   "report message preserves detail fields")
  (assert
   (not
    (nil?
     (wire-protocol:sexp-string
      (wire-protocol:make-report-message :executed :units items))))
...
(let* [{:reports planned-package-reports}
  ...
  (push stub-read-messages
        (stub-event :package
                    {:phase :packages :kind :installed :name "runtime-fail-pkg"}))
  (push stub-read-messages
        (stub-event :package
                    {:phase :packages :kind :installed :name "runtime-fail-dependent"}))
  (push stub-read-messages
        (stub-event :package
...
(let* [local-plan-names
       (list "elle-lsp-bridge")
       planned-package-reports
       (list (graph:make-report "elle-lsp-bridge" :ok :ready (list)))]
  (reset-stub-state)
  (push stub-read-messages
        (stub-event :package
...
(let* [decoded
  ...
  (push stub-read-messages
        (stub-event :package
                    {:phase :packages :kind :finished :reason "completed"}))
  (push stub-read-messages
        (stub-response 60 true
                       (list {:name "magit" :status :installed})))
  (execution:execute-package-entry-plan-tracker
   (planning:plan-names package-plan)
   planned-package-reports
   60)
...
(let* [planned-package-reports
  ...
  (push stub-read-messages
        (stub-response
         ...
         nil
         :error "clone failed"))
  (let* [{:reports package-reports :installed installed :next-id next-id}
         (execution:execute-package-entry-plan-tracker
          (list "consult-snapfile")
          planned-package-reports
          65)
    ...
    (assert
     (= (graph:entry-field (graph:entry-field snapfile :details) :error) "clone failed")
...
(let* [{:reports planned-package-reports}
  ...
  (push stub-read-messages
        (stub-response 66 false nil :error "runtime failed"))
  (let* [{:reports package-reports :installed installed :next-id next-id}
         (execution:execute-package-entry-plan-tracker
          (planning:plan-names package-plan)
          planned-package-reports
...
(let* [{:reports planned-package-reports}
  ...
  (let* [{:reports package-reports
    ...
    (assert (= (graph:entry-field (graph:entry-field ui :details) :error) "package install failed") "tracker failure error detail")
...
(let* [{:reports planned-package-reports}
  ...
  (let* [{:reports package-reports
    ...
    (assert
     (= error-detail "(void-function transient--set-layout)")
...
(let* [{:reports planned-package-reports}
  ...
  (push stub-read-messages
        (stub-event :package
                    {:phase :packages
                     :kind :finished
                     :reason "package directory permission denied"}))
  (push stub-read-messages
        (stub-response 70 false nil :error "package directory permission denied"))
  (let* [{:reports package-reports :installed installed :next-id next-id}
         (execution:execute-package-entry-plan-tracker
          (planning:plan-names package-plan)
          planned-package-reports
    ...
    (assert
     (= (graph:entry-field (graph:entry-field core :details) :error) "package directory permission denied")
...
(let* [many-packages
       (map
        ...
        (->list (range 0 80)))
       {:reports planned-package-reports}
       (policy:derive-package-reports many-packages)
  ...
  (push stub-read-messages
        (stub-event :package
                    {:phase :packages
                     ...
                     :reason "package directory permission denied"}))
  (push stub-read-messages
        (stub-response 80 false nil :error "package directory permission denied"))
  (let* [{:reports package-reports :installed installed :next-id next-id}
         (execution:execute-package-entry-plan-tracker
          (planning:plan-names package-plan)
          planned-package-reports
    ...
    (assert
     (= (graph:entry-field (first package-reports) :status) :failed)
     "tracker large nil batch first package fails")
    (assert
     (= (graph:entry-field (last package-reports) :status) :failed)
     "tracker large nil batch last package fails")))
...
(let* [{:reports planned-package-reports}
       ...
       (list
        ...
        (graph:make-report "runtime-fail-dependent" :skipped :blocked-by-package (list :blockers (list "runtime-fail-pkg")))
  ...
  (let* [{:reports unit-reports :next-id next-id}
    ...
    (assert (= (graph:entry-field (graph:entry-field runtime-fail :details) :source) :eval) "unit runtime failure source")
