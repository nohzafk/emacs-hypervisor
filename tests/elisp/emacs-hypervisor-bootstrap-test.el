...
(ert-deftest emacs-hypervisor-runtime-process-packages-reports-noop-packages-installed ()
  (let ((source-a (make-temp-file "emacs-hypervisor-source-a" t))
        (source-b (make-temp-file "emacs-hypervisor-source-b" t))
        (build-a (make-temp-file "emacs-hypervisor-build-a" t))
        (build-b (make-temp-file "emacs-hypervisor-build-b" t))
        (emacs-hypervisor-runtime-package-manager-ready t)
        (emacs-hypervisor-runtime-compat-enabled t)
        (emacs-hypervisor-runtime-package-timeout-seconds nil)
        (emacs-hypervisor-runtime-package-timeout-timer nil)
        (emacs-hypervisor-runtime-packages-installation-active nil)
    ...
    (unwind-protect
        (cl-letf (((symbol-function 'elpaca--queued)
                   (lambda ()
                     `((pkg-a . (:source ,source-a :build ,build-a :built nil :status queued))
                       (pkg-b . (:source ,source-b :build ,build-b :built nil :status queued)))))
                  ((symbol-function 'elpaca<-source-dir)
                   (lambda (entry)
                     (plist-get entry :source)))
                  ((symbol-function 'elpaca<-builtp)
                   (lambda (entry)
                     (plist-get entry :built)))
                  ((symbol-function 'elpaca<-build-dir)
                   (lambda (entry)
                     (plist-get entry :build)))
                  ((symbol-function 'elpaca--status)
                   (lambda (entry)
                     (plist-get entry :status)))
                  ((symbol-function 'elpaca-process-queues)
                   (lambda ()
                     ;; Elpaca can finish a no-op queue without running package
                     ;; body callbacks for packages that were already activated.
      ...
      (delete-directory source-a t)
      (delete-directory source-b t)
      (delete-directory build-a t)
      (delete-directory build-b t))
    (setq sent (nreverse sent))
    (should (equal emacs-hypervisor-installed-packages '("pkg-b" "pkg-a")))
    (should (equal sent
                   '((:package
                      (:phase :packages :kind :installed :name "pkg-a"))
                     (:package
                      (:phase :packages :kind :installed :name "pkg-b"))
                     (:package
                      (:phase :packages :kind :finished :reason "completed")))))))

(ert-deftest emacs-hypervisor-runtime-process-packages-finishes-on-elpaca-error ()
  (let ((source-a (make-temp-file "emacs-hypervisor-source-a" t))
        (source-b (make-temp-name "emacs-hypervisor-source-b"))
        (build-a (make-temp-file "emacs-hypervisor-build-a" t))
        (build-b (make-temp-name "emacs-hypervisor-build-b"))
        (emacs-hypervisor-runtime-package-manager-ready t)
        (emacs-hypervisor-runtime-compat-enabled t)
        (emacs-hypervisor-runtime-package-timeout-seconds nil)
        (emacs-hypervisor-runtime-package-timeout-timer nil)
    ...
    (should (equal emacs-hypervisor-installed-packages '("pkg-a")))
    (should (equal sent
                   '((:package
                      (:phase :packages :kind :installed :name "pkg-a"))
                     (:package
                      (:phase :packages :kind :finished :reason "(error \"missing source\")")))))))
