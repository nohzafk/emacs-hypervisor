...
(ert-deftest emacs-hypervisor-local-direct-p-detects-direct-local ()
  (emacs-hypervisor-reset-declarations)
  (package! direct-pkg :local "~/src/direct")
  (let ((entry (car (emacs-hypervisor-export-packages))))
    (should (emacs-hypervisor-bridge--local-direct-p entry))
    (should (equal (emacs-hypervisor-bridge--clone-dir entry)
                   (expand-file-name "~/src/direct")))))

(ert-deftest emacs-hypervisor-local-direct-clone-dir-is-source-not-staging ()
  (emacs-hypervisor-reset-declarations)
  (package! direct-pkg :local "~/src/direct")
  (package! vc-pkg :repo "example/vc-pkg")
  (let* ((entries (emacs-hypervisor-export-packages))
         (direct (car entries))
         (vc (cadr entries)))
    (should (string-prefix-p (expand-file-name "~/src/direct")
                             (emacs-hypervisor-bridge--clone-dir direct)))
    (should-not (string-prefix-p (expand-file-name "~/src/direct")
                                 (emacs-hypervisor-bridge--clone-dir vc)))
...
;; Mermaid extension tests removed — mermaid extracted to ~/projects/elle-mermaid/
