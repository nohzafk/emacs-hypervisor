use std::sync::Mutex;
use serde::{Deserialize, Serialize};
use wasm_bindgen::prelude::*;

lazy_static::lazy_static! {
    static ref GLOBAL_STATE: Mutex<HudState> = Mutex::new(HudState::default());
    static ref REPAINT_SIGNAL: Mutex<Option<egui::Context>> = Mutex::new(None);
    static ref GLOBAL_THEME: Mutex<ThemeColors> = Mutex::new(ThemeColors::default());
}

#[derive(Default, Serialize, Deserialize, Clone)]
pub struct UnitInfo {
    pub name: String,
    pub status: String,
}

#[derive(Serialize, Deserialize, Clone)]
pub struct HudState {
    #[serde(default = "default_branch")]
    pub branch: String,
    #[serde(default)]
    pub changes: String,
    #[serde(rename = "mcp-online", default)]
    pub mcp_online: bool,
    #[serde(default)]
    pub units: Vec<UnitInfo>,
    #[serde(default)]
    pub location: String,
    #[serde(rename = "last-commit", default)]
    pub last_commit: String,
    #[serde(rename = "gh-available", default)]
    pub gh_available: bool,
    #[serde(rename = "project-name", default)]
    pub project_name: String,
    #[serde(rename = "project-root", default)]
    pub project_root: String,
}

fn default_branch() -> String {
    "main".to_string()
}

impl Default for HudState {
    fn default() -> Self {
        Self {
            branch: "main".to_string(),
            changes: "0 files".to_string(),
            mcp_online: false,
            units: Vec::new(),
            location: String::new(),
            last_commit: String::new(),
            gh_available: false,
            project_name: String::new(),
            project_root: String::new(),
        }
    }
}

#[derive(Serialize, Deserialize, Clone)]
pub struct ThemeColors {
    #[serde(default)]
    pub bg: String,
    #[serde(default)]
    pub fg: String,
}

impl Default for ThemeColors {
    fn default() -> Self {
        Self {
            bg: "#0c0c10".to_string(),
            fg: "#e6ebff".to_string(),
        }
    }
}

fn parse_hex_color(hex: &str) -> Option<egui::Color32> {
    let hex = hex.trim_start_matches('#');
    if hex.len() >= 6 {
        let r = u8::from_str_radix(&hex[0..2], 16).ok()?;
        let g = u8::from_str_radix(&hex[2..4], 16).ok()?;
        let b = u8::from_str_radix(&hex[4..6], 16).ok()?;
        Some(egui::Color32::from_rgb(r, g, b))
    } else {
        None
    }
}

fn luminance(c: egui::Color32) -> u8 {
    ((c.r() as u32 * 299 + c.g() as u32 * 587 + c.b() as u32 * 114) / 1000) as u8
}

fn card_fill_from_bg(bg: egui::Color32) -> egui::Color32 {
    if luminance(bg) < 128 {
        // Dark theme: card slightly lighter
        egui::Color32::from_rgba_unmultiplied(
            bg.r().saturating_add(12),
            bg.g().saturating_add(12),
            bg.b().saturating_add(14),
            240,
        )
    } else {
        // Light theme: card is white-ish
        egui::Color32::from_rgba_unmultiplied(255, 255, 255, 245)
    }
}

fn muted_color(fg: egui::Color32) -> egui::Color32 {
    // Reduce opacity for muted text
    egui::Color32::from_rgba_unmultiplied(fg.r(), fg.g(), fg.b(), 128)
}

pub struct HudApp {}

impl HudApp {
    pub fn new(_cc: &eframe::CreationContext<'_>) -> Self {
        Self {}
    }
}

impl eframe::App for HudApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        // Store context for programmatic repaints when state is pushed
        if let Ok(mut signal) = (*REPAINT_SIGNAL).lock() {
            if signal.is_none() {
                *signal = Some(ctx.clone());
            }
        }

        let state = {
            if let Ok(guard) = (*GLOBAL_STATE).lock() {
                guard.clone()
            } else {
                HudState::default()
            }
        };

        let theme = {
            if let Ok(guard) = (*GLOBAL_THEME).lock() {
                guard.clone()
            } else {
                ThemeColors::default()
            }
        };

        let bg = parse_hex_color(&theme.bg).unwrap_or(egui::Color32::from_rgb(12, 12, 16));
        let fg = parse_hex_color(&theme.fg).unwrap_or(egui::Color32::from_rgb(230, 235, 255));
        let is_dark = luminance(bg) < 128;

        // Derived colors
        let col_text_primary = fg;
        let col_text_muted = muted_color(fg);
        let col_green = egui::Color32::from_rgb(100, 220, 100);
        let col_orange = egui::Color32::from_rgb(240, 150, 80);
        let col_red = egui::Color32::from_rgb(220, 80, 80);
        let card_bg = card_fill_from_bg(bg);
        let stroke_color = if is_dark {
            egui::Color32::from_rgba_unmultiplied(60, 60, 80, 100)
        } else {
            egui::Color32::from_rgba_unmultiplied(200, 200, 210, 100)
        };

        // Make the entire background transparent so only the card shows
        let mut style = (*ctx.style()).clone();
        style.visuals.widgets.noninteractive.bg_fill = egui::Color32::TRANSPARENT;
        style.visuals.window_fill = egui::Color32::TRANSPARENT;
        style.visuals.panel_fill = egui::Color32::TRANSPARENT;
        ctx.set_style(style);

        egui::CentralPanel::default()
            .frame(egui::Frame::new().fill(bg).inner_margin(egui::Margin::same(6)))
            .show(ctx, |ui| {
                // Card frame
                egui::Frame::new()
                    .fill(card_bg)
                    .corner_radius(egui::CornerRadius::same(12))
                    .inner_margin(egui::Margin::same(16))
                    .stroke(egui::Stroke::new(1.0, stroke_color))
                    .show(ui, |ui| {
                        // ── Project root (debug) ────────────────────────────
                        // Shows what repo root the HUD resolved, so we can see
                        // at a glance whether it is following the active buffer's
                        // git project. Falls back to the project name, then "—".
                        {
                            let root_display = if !state.project_root.is_empty() {
                                state.project_root.clone()
                            } else if !state.project_name.is_empty() {
                                state.project_name.clone()
                            } else {
                                "—".to_string()
                            };
                            ui.label(
                                egui::RichText::new(format!("⌂  {}", root_display))
                                    .size(10.0)
                                    .color(col_text_muted),
                            );
                            ui.add_space(6.0);
                        }

                        // ── Environment section header ──────────────────────
                        ui.horizontal(|ui| {
                            ui.label(
                                egui::RichText::new("Environment")
                                    .size(10.0)
                                    .color(col_text_muted)
                                    .strong(),
                            );
                            ui.with_layout(
                                egui::Layout::right_to_left(egui::Align::Center),
                                |ui| {
                                    ui.label(
                                        egui::RichText::new("⚙")
                                            .size(10.0)
                                            .color(col_text_muted),
                                    );
                                },
                            );
                        });

                        ui.add(egui::Separator::default().spacing(4.0));

                        // Row: Changes
                        {
                            let has_changes = !state.changes.is_empty()
                                && state.changes != "0 files"
                                && state.changes != "+0 -0";
                            let changes_display = if state.changes.is_empty() {
                                "0 files".to_string()
                            } else {
                                state.changes.clone()
                            };
                            let changes_color = if has_changes {
                                col_orange
                            } else {
                                col_text_muted
                            };
                            ui.horizontal(|ui| {
                                ui.label(
                                    egui::RichText::new("⊞  Changes")
                                        .size(12.0)
                                        .color(col_text_muted),
                                );
                                ui.with_layout(
                                    egui::Layout::right_to_left(egui::Align::Center),
                                    |ui| {
                                        ui.label(
                                            egui::RichText::new(&changes_display)
                                                .size(12.0)
                                                .color(changes_color),
                                        );
                                    },
                                );
                            });
                        }
                        ui.add_space(3.0);

                        // Row: Location
                        {
                            let loc = if state.location.is_empty() {
                                "Local".to_string()
                            } else {
                                state.location.clone()
                            };
                            ui.horizontal(|ui| {
                                ui.label(
                                    egui::RichText::new(format!("▪  {}", loc))
                                        .size(12.0)
                                        .color(col_text_muted),
                                );
                            });
                        }
                        ui.add_space(3.0);

                        // Row: Branch
                        ui.horizontal(|ui| {
                            ui.label(
                                egui::RichText::new(format!("⎇  {}", state.branch))
                                    .size(12.0)
                                    .color(col_green),
                            );
                        });
                        ui.add_space(3.0);

                        // Row: Last commit
                        {
                            let commit_display = if state.last_commit.is_empty() {
                                "no commits".to_string()
                            } else {
                                state.last_commit.clone()
                            };
                            ui.horizontal(|ui| {
                                ui.label(
                                    egui::RichText::new(format!("●  {}", commit_display))
                                        .size(12.0)
                                        .color(col_text_muted),
                                );
                            });
                        }
                        ui.add_space(3.0);

                        // Row: GitHub CLI
                        ui.horizontal(|ui| {
                            ui.label(
                                egui::RichText::new("●  GitHub CLI")
                                    .size(12.0)
                                    .color(col_text_muted),
                            );
                            ui.with_layout(
                                egui::Layout::right_to_left(egui::Align::Center),
                                |ui| {
                                    if state.gh_available {
                                        ui.label(
                                            egui::RichText::new("available")
                                                .size(12.0)
                                                .color(col_green),
                                        );
                                    } else {
                                        ui.label(
                                            egui::RichText::new("unavailable")
                                                .size(12.0)
                                                .color(col_text_muted),
                                        );
                                    }
                                },
                            );
                        });

                        ui.add_space(8.0);

                        // ── Sources section header ──────────────────────────
                        ui.horizontal(|ui| {
                            ui.label(
                                egui::RichText::new("Sources")
                                    .size(10.0)
                                    .color(col_text_muted)
                                    .strong(),
                            );
                        });

                        ui.add(egui::Separator::default().spacing(4.0));

                        // Row: Elle MCP
                        ui.horizontal(|ui| {
                            ui.label(
                                egui::RichText::new("⚙  Elle MCP")
                                    .size(12.0)
                                    .color(col_text_primary),
                            );
                            ui.with_layout(
                                egui::Layout::right_to_left(egui::Align::Center),
                                |ui| {
                                    if state.mcp_online {
                                        ui.label(
                                            egui::RichText::new("Online")
                                                .size(12.0)
                                                .color(col_green),
                                        );
                                    } else {
                                        ui.label(
                                            egui::RichText::new("Offline")
                                                .size(12.0)
                                                .color(col_red),
                                        );
                                    }
                                },
                            );
                        });

                        // Config units (if any)
                        if !state.units.is_empty() {
                            for unit in &state.units {
                                ui.add_space(3.0);
                                ui.horizontal(|ui| {
                                    ui.label(
                                        egui::RichText::new(format!("▸  {}", unit.name))
                                            .size(12.0)
                                            .color(col_text_primary),
                                    );
                                    ui.with_layout(
                                        egui::Layout::right_to_left(egui::Align::Center),
                                        |ui| {
                                            let (color, label) = match unit.status.as_str() {
                                                "running" => (col_green, "running"),
                                                "failed" => (col_red, "failed"),
                                                s => (col_text_muted, s),
                                            };
                                            ui.label(
                                                egui::RichText::new(label)
                                                    .size(12.0)
                                                    .color(color),
                                            );
                                        },
                                    );
                                });
                            }
                        }
                    });
            });
    }
}

#[cfg(target_arch = "wasm32")]
#[wasm_bindgen]
pub fn start(_canvas_id: &str) -> Result<(), JsValue> {
    // Redirect panics to browser console
    console_error_panic_hook::set_once();

    let web_options = eframe::WebOptions::default();
    wasm_bindgen_futures::spawn_local(async {
        let document = web_sys::window()
            .and_then(|win| win.document())
            .expect("Failed to get document");
        let canvas = document
            .get_element_by_id("hud-canvas")
            .expect("Failed to get canvas")
            .dyn_into::<web_sys::HtmlCanvasElement>()
            .expect("Failed to cast to canvas");

        eframe::WebRunner::new()
            .start(
                canvas,
                web_options,
                Box::new(|cc| Ok(Box::new(HudApp::new(cc)))),
            )
            .await
            .expect("failed to start eframe");
    });

    Ok(())
}

/// Fix type mismatches caused by the Emacs Lisp → JSON encoding layer.
/// Elle boolean `false` arrives as JSON string `"false"` instead of JSON boolean.
/// Elle empty list `()` arrives as JSON `null` instead of JSON array `[]`.
fn fixup_sexp_rpc_json(val: &mut serde_json::Value) {
    if let serde_json::Value::Object(map) = val {
        // Fix boolean fields that arrive as strings from Emacs json-encode
        for key in ["mcp-online", "gh-available"] {
            if let Some(v) = map.get(key).cloned() {
                if let Some(s) = v.as_str() {
                    match s {
                        "true" => { map.insert(key.to_string(), serde_json::Value::Bool(true)); }
                        "false" => { map.insert(key.to_string(), serde_json::Value::Bool(false)); }
                        _ => {}
                    }
                }
            }
        }
        // Fix array fields that arrive as null (Emacs nil for empty list)
        for key in ["units"] {
            if matches!(map.get(key), Some(serde_json::Value::Null)) {
                map.insert(key.to_string(), serde_json::Value::Array(vec![]));
            }
        }
        // Recurse into array elements (e.g. unit items)
        for (_, v) in map.iter_mut() {
            if let serde_json::Value::Array(arr) = v {
                for item in arr.iter_mut() {
                    fixup_sexp_rpc_json(item);
                }
            }
        }
    }
}

#[wasm_bindgen]
pub fn push_state(json: &str) {
    if let Ok(mut val) = serde_json::from_str::<serde_json::Value>(json) {
        fixup_sexp_rpc_json(&mut val);
        if let Ok(new_state) = serde_json::from_value::<HudState>(val) {
            if let Ok(mut guard) = (*GLOBAL_STATE).lock() {
                *guard = new_state;
            }
            // Trigger repaint immediately
            if let Ok(signal) = (*REPAINT_SIGNAL).lock() {
                if let Some(ctx) = &*signal {
                    ctx.request_repaint();
                }
            }
        }
    }
}

#[wasm_bindgen]
pub fn push_theme(json: &str) {
    if let Ok(new_theme) = serde_json::from_str::<ThemeColors>(json) {
        if let Ok(mut guard) = (*GLOBAL_THEME).lock() {
            *guard = new_theme;
        }
        if let Ok(signal) = (*REPAINT_SIGNAL).lock() {
            if let Some(ctx) = &*signal {
                ctx.request_repaint();
            }
        }
    }
}
