...
(defn emacs-hypervisor-planning-module [protocol graph]
    ...
    (filter
     (fn [entry]
       (let [report (graph:find-entry planned-reports (graph:entry-name entry))]
         (and (= (graph:entry-field report :status) :ok)
              (not (= (graph:entry-field report :reason) :installed)))))
     entries))
  ...
  (defn installed-entries [entries planned-reports]
    (filter
     (fn [entry]
       (let [report (graph:find-entry planned-reports (graph:entry-name entry))]
         (and (= (graph:entry-field report :status) :ok)
              (= (graph:entry-field report :reason) :installed))))
     entries))
  ...
  (defn derive-phase-plan [phase entries planned-reports dep-key]
    (let [plan-entries (planned-ok-entries entries planned-reports)]
      (letrec
          [loop
           (fn [remaining ready-names ordered-names ordered-items]
             (if (empty? remaining)
               {:names (reverse ordered-names)
                :items (reverse ordered-items)}
        ...
        (let [result
              (loop
               plan-entries
               (graph:known-names (installed-entries entries planned-reports))
               ()
               ())]
  ...
  (defn emit-plan-message [plan]
    (protocol:send-event
     :plan
     {:phase (graph:entry-field plan :phase)
  ...
  (defn merge-executed-reports [planned-reports executed-reports]
    (if (empty? executed-reports)
      ...
      (map
       (fn [planned-report]
         (or
          (graph:find-entry executed-reports (graph:entry-name planned-report))
          planned-report))
       planned-reports)))
