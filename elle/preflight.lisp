...
(defn emacs-hypervisor-preflight-module [_protocol graph _mailbox _benchmark]
  ...
  (defn path-entries [env]
    (if-let [path-value (env-entry-value env "PATH")]
      (string/split path-value ":")
      ()))

  (defn executable-path [binary path-dirs]
    (cond
     (string/starts-with? binary "/")
      (if (file/exists? binary) binary nil)
     true
      (letrec
          [loop
           (fn [dirs]
       ...
       (map
        (fn [unit]
          (let [binary (first (graph:entry-field unit :executable))]
            (executable-probe-report
             unit
             (probe-executable binary path-dirs))))
        units)}))
