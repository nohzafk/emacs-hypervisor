  ...
  (if (nil? value)
    ...
    (let [kind (type-of value)]
      (case kind
        ...
        :list (string "<list len=" (number->string (length value)) ">")
        :array (string "<array len=" (number->string (length value)) ">")
        :@array (string "<@array len=" (number->string (length value)) ">")
        :struct "<struct>"
        :@struct "<@struct>"
        (string "<" (string kind) ">")))))
           ...
           (fn [remaining index]
             (when (and (not (empty? remaining)) (< index limit))
               (let [report (first remaining)]
                 (eprintln
                  "[Hypervisor debug] report-item "
                  label
                  " index="
                  (number->string index)
                  " type="
                  (emacs-hypervisor-debug-value-summary report)
                  " raw="
...
(defn emacs-hypervisor-debug-send-report [stage phase reports]
  (let [label (string
               "stage="
               (emacs-hypervisor-debug-value-summary stage)
               " phase="
               (emacs-hypervisor-debug-value-summary phase))]
    (eprintln
     "[Hypervisor debug] report-send "
     label
     " type="
     (emacs-hypervisor-debug-value-summary reports)
     " count="
           ...
           (fn [remaining index]
             (when (and (not (empty? remaining)) (< index limit))
               (let [entry (first remaining)]
                 (eprintln
                  "[Hypervisor debug] source-package index="
                  (number->string index)
                  " type="
                  (emacs-hypervisor-debug-value-summary entry)
                  " raw="
...
(protocol:with-mailbox-reader
 ...
 (fn []
   ...
   (let* [boot-context-response (protocol:await-response mailbox 2)
          ...
          boot-config-file
          (and (get boot-context :config-file)
               (string (get boot-context :config-file)))
          boot-config-org-file
          (and (get boot-context :config-org-file)
          ...
          benchmark-enabled
          (and (not (nil? (get boot-context :benchmark-enabled)))
     ...
     (protocol:send-event
      :log
      `(:level :info
        :message "installing config surface"))
     (protocol:send-request
      3
      :eval
     ...
     (let [config-load-payload
           (benchmark:eval-payload
            (list
             :form
             '(emacs-hypervisor-load-startup-config)
