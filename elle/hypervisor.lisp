...
(def preflight (emacs-hypervisor-preflight-module protocol graph))
...
(def execution (emacs-hypervisor-execution-module protocol graph))
...
(let [hello-response (protocol:await-response 1)]
...
(let* [boot-context-response (protocol:await-response 2)
  ...
  (let* [session-data-response (protocol:await-response 3)
         ...
         executable-probes
         (preflight:probe-executables (preflight:units-with-executables units) 10)
    ...
    (let [runtime-result (protocol:await-response 4)]
