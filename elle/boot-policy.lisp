...
(defn emacs-hypervisor-boot-policy-module [graph preflight]
  ...
  (defn derive-phase-reports [entries invalid-reports ready? next-report]
    (let [remaining-entries (graph:non-invalid-entries entries invalid-reports)
          unordered-reports (graph:resolve-reports-loop remaining-entries invalid-reports ready? next-report)]
  ...
  (defn next-unit-report [entry reports package-reports env executable-reports]
    (let [package-blockers (graph:known-report-blockers package-reports (graph:entry-field entry :requires))
          unit-blockers (graph:report-blockers reports (graph:entry-field entry :after))
    ...
    (let [unit-names (graph:known-names units)
      ...
      {:cycles unit-cycles
       ...
       :reports (derive-phase-reports units invalid-unit-reports
                                      ...
                                      (fn [entry reports]
                                        (next-unit-report entry reports package-reports env executable-reports)))}))

  (defn init-upgrade-command [binary home]
    (string (or binary "emacs-hypervisor") " init --home " (or home "~/.config/emacs") " --upgrade"))
  ...
  (defn bootstrap-warning [boot-context expected-hash]
    (let [current-hash (get boot-context :init-content-hash)
      ...
      (cond
        (not generated?) nil
        (nil? expected-hash) nil
        (nil? current-hash) {:kind :bootstrap-hash
                             :level :warning
                             ...
                             :home home
                             :init-file init-file}
        (not (= current-hash expected-hash)) {:kind :bootstrap-hash
                                              :level :warning
