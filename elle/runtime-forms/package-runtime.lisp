## Emitted package/runtime forms for Elpaca queueing and tracker behavior.

(defn emacs-hypervisor-runtime-forms-package-module []
  (defn session-policy-forms []
    '((defvar emacs-hypervisor-runtime-compat-enabled nil)
      (defvar emacs-hypervisor-runtime-package-timeout-seconds 30)
      ...
      (defun emacs-hypervisor-runtime-start-package-timeout ()
        ...
        (setq emacs-hypervisor-runtime-package-timeout-timer
              (run-at-time emacs-hypervisor-runtime-package-timeout-seconds
                           nil
                           (function emacs-hypervisor-runtime-package-timeout))))
  ...
  (defn compatibility-install-forms []
    '((emacs-hypervisor-elpaca-bootstrap)
      (unless emacs-hypervisor-runtime-compat-enabled
         (add-hook 'elpaca-recipe-functions
                   (function emacs-hypervisor-elpaca-infer-main-file))
         (advice-add 'elpaca--shared-source-dir
                     :around
                     (function emacs-hypervisor-elpaca-shared-source-dir))
         (advice-add 'elpaca-source
                     :around
                     (function emacs-hypervisor-elpaca-wait-for-shared-git-source))
         (advice-add 'elpaca-git--clone
                     :around
                     (function emacs-hypervisor-elpaca-wait-on-shared-main-before-clone))
         (setq emacs-hypervisor-runtime-compat-enabled t))
       (when (boundp 'elpaca--post-queues-hook)
         (remove-hook 'elpaca--post-queues-hook
                      (function emacs-hypervisor-runtime-queues-finished))
         (add-hook 'elpaca--post-queues-hook
                   (function emacs-hypervisor-runtime-queues-finished)))))

  {:compatibility-install-forms compatibility-install-forms
   :runtime-forms runtime-forms
   :session-policy-forms session-policy-forms
